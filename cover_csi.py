...
def read_log_file(file_name):
    ...
    path_name='output_rssi.txt'
    try:
        file_print= open(path_name,'w')
    except:
        print('cant open file: output.csv')
        exit
    file_print.write('timestamp,RSSI,RSSI1,RSSI2,RSSI3\n')    
    while cur<(len_file-4):
        ...
        cur+=1
        file_print.write('%d,' % (rssi))

        rssi1 = fromfileskip(f, 1, 'uint8')
        cur+=1
        file_print.write('%d,' % (rssi1))

        rssi2 = fromfileskip(f, 1, 'uint8')
        cur+=1
        file_print.write('%d,' % (rssi2))
